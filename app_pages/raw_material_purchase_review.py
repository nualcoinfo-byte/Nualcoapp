...
st.title("Purchase Invoice Review")
st.caption(
    "Every raw material invoice logged as **Pending with purchase**. "
    "Review each one, then send it on to Accounts or cancel it."
)

pending = db.list_raw_material_purchases_by_status("Pending with purchase")

if not pending:
    st.info("No invoices are pending with purchase.")
else:
    summary_rows = [
        {
    ...
    for row in pending:
        ...
        with st.expander(header):
            ...
            if accept_col.button(
                "Submit to Accounts",
                key=f"purch_review_accounts_{purchase_id}",
                type="primary",
            ):
                try:
                    db.set_raw_material_purchase_invoice_status(
                        purchase_id, "Pending with accounts"
                    )
                    st.success(f"Invoice #{purchase_id} sent to Accounts.")
                    st.rerun()
                except Exception as exc:
                    st.error(f"Could not submit: {exc}")
            if cancel_col.button(
                "Cancel Invoice",
                key=f"purch_review_cancel_{purchase_id}",
            ):
                try:
                    db.set_raw_material_purchase_invoice_status(purchase_id, "Cancelled")
                    st.success(f"Invoice #{purchase_id} cancelled.")
                    st.rerun()
                except Exception as exc:
