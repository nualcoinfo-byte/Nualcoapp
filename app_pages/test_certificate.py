...
CERT_SYSTEM_NOTE = (
    "This is a system-generated test certificate and requires no physical signature."
)
...
def _certificate_pdf_bytes(payload: dict) -> bytes:
    ...
    def _draw_page(chunk: list[dict], offset: int, page_no: int, page_count: int) -> None:
        ...
        # Heat No / Kgs / Pcs / ACTUAL % each get their own 5 mm row: with 5 heats
        # a column is ~25 mm, too narrow for "Kgs : ...  Pcs : ..." on one line.
        head_rows = 4
        head_h = 5.0 * head_rows
        row_h = 5.0
        y0 = pdf.get_y()
        ...
        pdf.set_xy(left, y0 + (head_h - 5) / 2)
        pdf.cell(elem_w, 5, "ELEMENTS", align="C")
        pdf.set_xy(left + elem_w, y0 + (head_h - 5) / 2)
        ...
        for index, heat in enumerate(chunk or [{"Heat_no": "—", "Kgs": "", "Pieces": ""}]):
            ...
            for row_no, text in enumerate(
                (
                    f"Heat No : {heat.get('Heat_no') or '—'}",
                    f"Kgs : {kgs}",
                    f"Pcs : {pcs}",
                    "ACTUAL %",
                )
            ):
                pdf.rect(x, y0 + 5 * row_no, heat_w, 5, style="DF")
                pdf.set_xy(x, y0 + 5 * row_no)
                pdf.cell(heat_w, 5, _fit(text, heat_w), align="C")
        pdf.set_y(y0 + head_h)
        ...
        pdf.ln(6)
        pdf.set_font("Helvetica", "", 8)
        pdf.cell(page_w, 5, _pdf_safe_text(_generated_by_text(payload)))
        pdf.set_font("Helvetica", "I", 7)
        # Bottom right corner, inside the same 10 mm printable edge as the sides.
        pdf.set_xy(left, pdf.h - 10 - 5)
        pdf.cell(page_w, 5, _pdf_safe_text(CERT_SYSTEM_NOTE), align="R")
...
def _generated_by_text(payload: dict) -> str:
    who = str(payload.get("generated_by") or "").strip() or "—"
    when = str(payload.get("generated_at") or "").strip()
    return f"Generated by : {who}" + (f"  ({when})" if when else "")
...
def _render_certificate_print(
    ...
    employee = st.session_state.get("auth_employee")
    payload["generated_by"] = (
        db.employee_display_name(employee) if employee else db.get_acting_user()
    )
    payload["generated_at"] = db.now_ist().strftime("%d-%m-%Y %H:%M")
    letterhead = _company_letterhead(payload.get("company") or {})
    ...
    for page_no, (offset, chunk) in enumerate(pages, start=1):
        ...
        <div class="tc-doc">
            ...
            <div class="tc-foot">
                <span>{esc(_generated_by_text(payload))}</span>
                <span class="tc-system-note">{esc(CERT_SYSTEM_NOTE)}</span>
            </div>
        </div>
        """
    st.markdown(
        ...
        .tc-foot {{
            ...
            gap: 1rem; margin-top: 0.6rem; font-size: 0.8rem;
        }}
        .tc-system-note {{ text-align: right; font-style: italic; color: #444; }}
        </style>
        {docs}
        """,
