    ...
    *,
    ...
    if cert and cert.get("Status") == CERT_STATUS_ISSUED:
        if lines_changed or status_changed:
            raise ValueError(
                "Void the issued test certificate before changing packed "
                "batches or packing-list status."
            )
    ...
    *,
    ...
    if existing and existing.get("Status") == CERT_STATUS_ISSUED:
        raise ValueError(
            ...
            "Void it before creating a new draft."
        )
...
def save_packing_list_certificate_draft(
    ...
    *,
    ...
    if existing.get("Status") == CERT_STATUS_ISSUED:
        raise ValueError("Issued test certificates cannot be edited. Void it first.")
    header = get_packing_list(packing_list_id)
...
def void_packing_list_certificate(packing_list_id: int) -> dict[str, Any]:
    _ensure_packing_list_ready()
    existing = get_packing_list_certificate(packing_list_id)
    if not existing:
    ...
    if not voided:
        raise ValueError("Could not void the test certificate.")
    return voided
